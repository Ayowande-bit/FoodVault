...
export default function PaymentVerify() {
    ...
    useEffect(() => {
        ...
        const planId = searchParams.get('planId'); // Passed in callback URL
        ...
        const verifyPayment = async () => {
            try {
                ...
                if (data.status) { // Assuming api returns { status: true, ... }
                    setStatus('success');
                    setMessage('Payment verified successfully!');

                    // Optional: If we had a planId, we could update local storage here if needed, 
                    // but best to let Dashboard fetch fresh data.
                    // If the backend doesn't automatically credit the plan on verify, we might need an extra call,
                    // but usually 'verify' completes the transaction.
